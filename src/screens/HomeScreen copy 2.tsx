...
import { NewsArticle, TalkshowEntry, TalkshowResponse } from '../type';
...
type SectionItem =
  ...
  | { type: 'talkshow'; data: { showSlug: string; talkshow: any }[] }
  | { type: 'radio' }
...
export default function HomeScreen({ navigation }: Props) {
  ...
  //const [talkshowData, setTalkshowData] = useState<TalkshowResponse['talkshow']>({});
  const [talkshowData, setTalkshowData] = useState<Record<string, TalkshowEntry>>({});

  
  // Initialize ads hook
  const { showInterstitialAd, isInterstitialLoaded } = useAds();

  // Pause all audio when Home screen comes into focus
  useFocusEffect(
    React.useCallback(() => {
      // Pause any audio that might be playing from other screens
  ...
  const liveTalkshows = Object.entries(talkshowData)
    .filter(([_, talkshow]) => talkshow.live)
    .map(([showSlug, talkshow]) => ({ 
      showSlug: showSlug,
      talkshow 
    }));
  ...
  const sections: SectionItem[] = [
    { type: 'adBanner' },
    { type: 'featured', data: featured },
    { type: 'borderNewsList', data: borderNewsList },
    { type: 'newsList', data: newsList },
    // Conditionally render talkshow or lastNewsList based on live shows
  ...
  // Handle navigation with interstitial ad
  const handleNewsNavigation = async (postId: number, title: string) => {
    try {
      // Show interstitial ad if available (30% chance to avoid overwhelming users)
          ...
          <View style={styles.section}>
            {item.data.map((show, idx) => (
              <TalkshowCard
                ...
                onPress={() => {
                  // Navigation to be added if needed
                }}
              />
            ))}
          </View>
