...
class UniversalAudioService {
  ...
  private async _loadLiveStreamsInternal(): Promise<AudioTrack[]> {
    try {
      ...
      console.log(`UniversalAudioService: Fetched ${playListData.audio?.length || 0} stream(s) from CDN`);

      if (!playListData.audio || playListData.audio.length === 0) {
        throw new Error('No audio streams found in playlist data');
      }

      // Create track objects
      const tracks: AudioTrack[] = playListData.audio
        .map((track, index) => {
          const audioTrack = {
            id: `live_${track.id}`,
